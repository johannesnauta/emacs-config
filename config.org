...
change the height a bit as I find the default height to be slighly too large. I also
include a fall back font [for Greek symbols] as these do not seem to be included in my
desired font.
#+begin_src emacs-lisp :tangle yes  
  (set-face-attribute 'default nil :family "Monaspace Neon" :height 110 :weight 'medium)
  ;; Monaspace Neon does not support Greek symbols, it seems, so use Fira Code for that
  (set-fontset-font "fontset-default" 'greek "Fira Code")
  ...
  ;; (set-face-attribute 'default nil :family "Fira Code" :height 110 :weight 'medium)
  ;; (set-face-attribute 'default nil :family "JuliaMono" :height 105 :weight 'bold)
...
#+begin_src emacs-lisp :tangle yes
  ;; Set C-M-= and C-M-- to scale *all* frames
  ;; In essence, it's an Emacs-wide version of =text-scale-mode=
...
#+begin_src emacs-lisp :tangle yes
  (use-package poke-line
    ...
    :config
    (setq-default poke-line-pokemon "gastly")
...
#+begin_src emacs-lisp :tangle yes
  (use-package ghostel
    :ensure t
    :bind (("C-x m" . (lambda () (interactive) (ghostel t)))
           ("C-c r" . my/ghostel-send-line)
           ("C-c R" . my/ghostel-send-line-reset)
    ...
    (defun my/ghostel-send-C-k-and-kill ()
      "Send C-k to ghostel and copy killed text to Emacs kill-ring."
      (interactive)
    ...
    (defvar my/ghostel-target nil
      "Last ghostel buffer used as send target.")

    (defun my/ghostel--pick-buffer ()
      "Prompt for a ghostel buffer and remember it."
      (setq my/ghostel-target
            (completing-read "Send to: " (mapcar #'buffer-name (ghostel--all-buffers)))))

    (defun my/ghostel-send-line (&optional reset)
      "Send current line to the remembered ghostel buffer.
  With prefix arg or RESET, re-prompt for target."
      (interactive "P")
      (when (or reset
                (null my/ghostel-target)
                (not (get-buffer my/ghostel-target)))
        (my/ghostel--pick-buffer))
      (let ((text (string-trim (thing-at-point 'line t))))
        (display-buffer my/ghostel-target)
        (with-current-buffer my/ghostel-target
          (ghostel-send-string (concat text "\n")))))

    (defun my/ghostel-send-line-reset ()
      "Re-prompt for target, then send current line."
      (interactive)
      (my/ghostel-send-line t)))
#+end_src
...
* LSP and related functionality: gotta code fast
...
#+begin_src emacs-lisp :tangle yes
  (use-package tex
    ...
    (setq TeX-view-program-list
      '(("Zathura" "zathura --synctex-forward %n:%b:%o --reuse-window &")))
...
*** =python-mode=
This mode is the bread-and-butter of using Python in Emacs. It's old, it's realiable (or
at least, seems to be), and the documentation is bad. But, it gives a lot of useful
functionality out-of-the-box, such as sending lines to the REPL, etc. Here, we just
trigger it to ensure it is activated, as is =eglot=.
#+begin_src emacs-lisp :tangle no
...
*** =python-ts-mode=
It lives in the built-in =python.el= package, so normally this one does not need to be
installed at all. We can specifically tell =use-package= to not install it. Finally, we
also trigger =eglot= whenever a =.py= file is opened.
#+begin_src emacs-lisp :tangle yes
  (use-package python
    :defer t
    ;; Built-in, so do not install
    :ensure nil
    :mode "\\.py\\'"
...
*** multi-vterm and Python
What a mess
#+begin_src emacs-lisp :tangle no
  (defvar multi-vterm-get-python-vterm "*python-vterm*"
    "Name of the multi-vterm buffer used for Python REPL.")

  (defun multi-vterm-create-python-vterm ()
    (interactive)
    "Return the multi-vterm buffer used for Python.
         If it doesn't exist, create it and rename to `my-python-vterm-name`."
    (or (get-buffer multi-vterm-get-python-vterm)
        (let ((currentbuffer (multi-vterm))) ; create a new multi-vterm
          (with-current-buffer currentbuffer
            (rename-buffer multi-vterm-get-python-vterm))
          currentbuffer)))

  (defun multi-vterm-send-region-to-python (start end)
    "Send the current region to the Python vterm buffer."
    (interactive "r")
    (let* ((buf (multi-vterm-create-python-vterm))
  ...
  (defun multi-vterm-send-line-to-python ()
    "Send the current line to the Python vterm buffer."
    (interactive)
    (multi-vterm-send-region-to-python (line-beginning-position) (line-end-position)))

  (defun multi-vterm-python-set-keys ()
    (interactive)
    "Set up keybindings for sending code to Python vterm."
    (local-set-key (kbd "C-c C-l") 'multi-vterm-send-line-to-python)
    (local-set-key (kbd "C-c C-r") 'multi-vterm-send-region-to-python))

  (add-hook 'python-mode-hook 'multi-vterm-python-set-keys)
#+end_src
...
=markdown-ts-mode=
The treesitter mode for Markdown. It will go core in Emacs31, but seeing that =brew
install --cask emacs= still defaults to Emacs30 put this here.
#+begin_src emacs-lisp :tangle yes
...
*** Remnants
these can be set as sometimes the treesit-language grammars are not found [?]
#+begin_src emacs-lisp :tangle no
...
*** Julia
We create a recipe and specify that we should always switch to =julia-ts-mode= if possible.
#+begin_src emacs-lisp :tangle no
...
*** Python
Install LSP for Python, here, =lsp-pyright= is used in conjunction with Ruff for
formatting.
#+begin_src emacs-lisp :tangle yes
